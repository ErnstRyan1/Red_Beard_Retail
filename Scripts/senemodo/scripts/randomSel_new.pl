...
#(10-12-15 fix) : made it so that the script remembers the percentage so you don't have to type it in every time.
...
if (lxq("query scriptsysservice userValue.isdefined ? sene_randomSel") == 0)
{
	...
	lx( "user.defNew sene_randomSel type:[float] life:[config]");
