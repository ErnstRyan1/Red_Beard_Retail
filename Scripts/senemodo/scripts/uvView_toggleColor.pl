...
#SETUP USER VALUES
userValueTools(seneUVColorToggle,boolean,config,"seneUVColorToggle","","","",xxx,xxx,"","");
lxout("sdfsdf");
#TOGGLE VIEWPORT COLOR
if (lxq("user.value seneUVColorToggle ?") == 1)	{	lx("viewport.scheme red.3d");	lx("user.value seneUVColorToggle 0");	}
else											{	lx("viewport.scheme uv.3d");	lx("user.value seneUVColorToggle 1");	}
