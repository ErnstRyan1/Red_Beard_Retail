...
# selSameSmoothingGroup : select all visible polys in layer that are sharing the same smoothing groups as those polys that are selected.
# selAllPolysWithSmGroups : select all visible polys in layer that have any smoothing groups assigned at all.
# selPolysBySize : select or deselect polys based off of their physical size
# selMeshIslsByBBOXSize : select or deselect mesh islands based off of their physical size
# deselPolyEdges : will deselect all the edges that the selected polys use.
# addPolyBoundEdges : identical to modo's normal boundary edge selection, only it automatically puts you in poly sel mode first so it'll actually work.
...
#(11-13-14 bugfix) : my select concave/convex edges code had a bug where it only worked with geometry of a certain size because i forgot to normalize a vector.  all fixed.
#(4-21-15 feature) : selSameSmoothingGroup : select all visible polys in layer that are sharing the same smoothing groups as those polys that are selected.
#(4-21-15 feature) : selAllPolysWithSmGroups : select all visible polys in layer that have any smoothing groups assigned at all.
#(11-20-15 feature) : selPolysBySize : select or deselect polys based off of their physical size
#(11-20-15 feature) : selMeshIslsByBBOXSize : select or deselect mesh islands based off of their physical size
#(11-20-15 feature) : deselPolyEdges : will deselect all the edges that the selected polys use.
#(11-20-15 feature) : addPolyBoundEdges : identical to modo's normal boundary edge selection, only it automatically puts you in poly sel mode first so it'll actually work.
...
foreach my $arg (@ARGV){
	...
	elsif	($arg =~ /deselBoundaryEdges/i)			{	deselBoundaryEdges();			}
	...
	elsif	($arg =~ /selEdgesByLength/i)			{	selEdgesByLength();				}
	elsif	($arg =~ /selSameSmoothingGroup/i)		{	selSameSmoothingGroup();		}
	elsif	($arg =~ /selAllPolysWithSmGroups/i)	{	selAllPolysWithSmGroups();		}
	elsif	($arg =~ /selPolysBySize/i)				{	selPolysBySize();				}
	elsif	($arg =~ /selMeshIslsByBBOXSize/i)		{	selMeshIslsByBBOXSize();		}
	elsif	($arg =~ /deselPolyEdges/i)				{	deselPolyEdges();				}
	elsif	($arg =~ /addPolyBoundEdges/i)			{	addPolyBoundEdges();			}
...
#------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------
#SELECT BOUNDARY POLYGON EDGES
#------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------
sub addPolyBoundEdges{
...
#------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------
#DESELECT EDGES FROM SELECTED POLYS
#------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------
sub deselPolyEdges{
	...
	foreach my $poly (@polys){	
		...
		for (my $i=0; $i<@vertList; $i++){
			$edgeTable{$vertList[$i].",".$vertList[$i-1]} = 1;
		}
	}
...
#------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------
#SELECT MESH ISLANDS BY SIZE
#------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------
sub selMeshIslsByBBOXSize{
	if ($miscArg eq "desel")	{	our @polys = lxq("query layerservice polys ? selected");	our $selMode = "remove";	}
	else						{	our @polys = lxq("query layerservice polys ? visible");		our $selMode = "add";		}
	
	my %alreadyCheckedPolys;
	my $sizeAndMode = quickDialog("Size To Select","string","<.5 or >.5 or .5>=<1","","");
	
	my $lesserOrGreater;
	if		($sizeAndMode =~ />=</)	{	$lesserOrGreater = ">=<";											}
	elsif	($sizeAndMode =~ /</)	{	$lesserOrGreater = "<";												}
	elsif	($sizeAndMode =~ />/)	{	$lesserOrGreater = ">";												}
	else							{	die("You didn't type in a '>' or '<' so I'm canceling the script");	}
	
	my @numbers = $sizeAndMode =~ m/[0-9.]+/g;
	
	foreach my $poly (@polys){
		if ($alreadyCheckedPolys{$poly} != 1){

			my @touchingElemRefs = listTouchingPolysAndVerts($poly);
			$alreadyCheckedPolys{$_} = 1 for (keys %{$touchingElemRefs[0]});
			
			my @bbox = boundingbox((keys %{$touchingElemRefs[1]}));
			my $sizeX = $bbox[3] - $bbox[0];
			my $sizeY = $bbox[4] - $bbox[1];
			my $sizeZ = $bbox[5] - $bbox[2];
			my $largestDimension = $sizeX;
			if ($sizeY > $largestDimension)	{	$largestDimension = $sizeY;	}
			if ($sizeZ > $largestDimension)	{	$largestDimension = $sizeZ;	}
			
			if		($lesserOrGreater eq "<"){
				if ($largestDimension < $numbers[0]){
					lx("!!select.element $mainlayer polygon $selMode $_") for (keys %{$touchingElemRefs[0]});
				}
			}elsif	($lesserOrGreater eq ">"){
				if ($largestDimension > $numbers[0]){
					lx("!!select.element $mainlayer polygon $selMode $_") for (keys %{$touchingElemRefs[0]});
				}
			}elsif	($lesserOrGreater eq ">=<"){
				if ( ($largestDimension > $numbers[0]) && ($largestDimension < $numbers[1]) ){
					lx("!!select.element $mainlayer polygon $selMode $_") for (keys %{$touchingElemRefs[0]});
				}
			}
		}
...
#------------------------------------------------------------------------------------------------------------
sub selPolysBySize{
	if ($miscArg =~ /desel/i)	{	our @polys = lxq("query layerservice polys ? selected");	our $selMode = "remove";	}	
	else						{	our @polys = lxq("query layerservice polys ? visible");		our $selMode = "add";		}
	my $sizeString = quickDialog("Poly Size?",string,"<.5 or >.5 or 0>=<.5","","");
	my $mode = -1; #0:< 1:> 2:<=>
	my @numbers = split (/[^0-9.]+/, $sizeString);
	if ($sizeString =~ /=/)		{	$mode = 2;	}
	elsif ($sizeString =~ /</)	{	$mode = 0;	}
	elsif ($sizeString =~ />/)	{	$mode = 1;	}
	else						{	die("The input text didn't see any < or > character and so it doesn't know what to do");	}
	
	foreach my $poly (@polys){
		my $area = getPolyArea($poly);
		if		( ($mode == 0) && ($area < $numbers[1]) )								{	lx("select.element $mainlayer polygon $selMode $poly");	}
		elsif	( ($mode == 1) && ($area > $numbers[1]) )								{	lx("select.element $mainlayer polygon $selMode $poly");	}
		elsif	( ($mode == 2) && ($area <= $numbers[2]) && ($area => $numbers[1]) )	{	lx("select.element $mainlayer polygon $selMode $poly");	}
	}
}

#------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------
#SEL ALL VISIBLE POLYS WITH ANY SMOOTHING GROUPS
#------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------
sub selAllPolysWithSmGroups{
...
#------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------
#SEL ALL VISIBLE POLYS WITH SAME SMOOTHING GROUP
#------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------
sub selSameSmoothingGroup{
	my %smoothingGroups;
	my @polys = lxq("query layerservice polys ? selected");
	foreach my $poly (@polys){
		my @tagTypes = lxq("query layerservice poly.tagTypes ? $poly");
		for (my $i=0; $i<@tagTypes; $i++){
			if ($tagTypes[$i] eq "SMGP"){
				...
				$smoothingGroups{$tags[$i]} = 1;
			}
		}
	}
	
	foreach my $tag (keys %smoothingGroups){
		lx("select.polygon add smoothGroup face {$tag}");
	}
}

#------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------
#DESELECT BOUNDARY EDGES
#------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------
sub deselBoundaryEdges{
	my @edges = lxq("query layerservice edges ? selected");
	lx("!!select.drop edge");
	lx("!!select.type polygon");
	lx("!!select.boundary");
	my @newEdges = lxq("query layerservice edges ? selected");
	my @finalEdges = removeListFromArray(\@edges,\@newEdges);
	lx("!!select.drop edge");
	foreach my $edge (@finalEdges){
...
#------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------
sub selEdgesByLength{
	my @edges = lxq("query layerservice edges ? visible");
	my $sizeString = quickDialog("Select edges of this size",string,"<0.5","","");
	my $mode = 0;
	if ($sizeString =~ /</)	{	$mode = 0;	}
	else					{	$mode = 1;	}
	my $size = $sizeString;
	$size =~ s/[^0-9.]//g;
	...
	if ($mode == 0){
		foreach my $edge (@edges){
			if 	(lxq("query layerservice edge.length ? $edge") <= $size){
				my @verts = split (/[^0-9]/, $edge);
				lx("!!select.element $mainlayer edge add $verts[1] $verts[2]");
			}
		}
...
#------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------
#SELECT CONCAVE OR CONVEX EDGES (simple but flawed because it queries poly positions.)
#------------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------
...
sub getThreeNonColinearVertsFromNgon{
	...
	my @vertList = lxq("query layerservice poly.vertList ? $_[0]");
	if (@vertList < 3){	die("getThreeNonColinearVertsFromNgon : This poly {$_[0]} does not have over 2 verts so I'm cancelling the script");	}
	...
	if (abs($dp) > 0.99999){	$foundColinearEdge = 1;	}
	...
	if ($foundColinearEdge == 0){	return ($vertList[0],$vertList[1],$vertList[2]);	}
		...
		for (my $i=3; $i<@vertList; $i++){
			...
			if ((abs($vector1[0]) == 0) && (abs($vector1[1]) == 0) && (abs($vector1[2]) == 0)){die("getThreeNonColinearVertsFromNgon : This poly {$_[0]} has 2 verts lying on top of each other so I'm cancelling script");}
			my $dp = dotProduct(\@vector0,\@vector1);
			if (abs($dp) < 0.99999){	return($vertList[0],$vertList[1],$vertList[$i]);	}
		}
	}
