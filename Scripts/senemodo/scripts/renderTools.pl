...
#(6-11-14 fix) : fixed an actr storage bug.
#(1-4-15 feature) : added fovTweak and focalLenTweak so you can adjust the camera's fov and target distance from the camera viewport if you bind your keys like i did.
#(4-2-15 fix) : added proxies to the list of items that can be selected by the SELECTALLBYTYPE sub
#(11-24-15 fix) : added lattice deformers to the list of items that can be selected by the SELECTALLBYTYPE sub
...
userValueTools(sene_occlusionRays,integer,config,sene_occlusionRays,"","","",0,4096,"",64);
...
foreach my $arg (@ARGV){
	...
	elsif	($arg =~ /occlusionTexRays/i)	{	&occlusionTexRays;		}
	...
	elsif	($arg =~ /bakeEachImage/i)		{	&bakeEachImage;			}
	elsif	($arg =~ /setRenderReg_selUVs/i){	&setRenderReg_selUVs;	}
	elsif	($arg =~ /turnOffRenderRegion/i){	&turnOffRenderRegion;	}
	elsif	($arg =~ /lightEditor/i)		{	&lightEditor;			}
	elsif	($arg =~ /fovTweak/i)			{	&fovTweak;				}
	elsif	($arg =~ /focalLenTweak/i)		{	&focalLenTweak;			}
	elsif	($arg =~ /setupRenderOutputs/i)	{	&setupRenderOutputs;	}
	...
	elsif	($arg eq "skipForward")			{	our $skipForward = 1;	}
	elsif	($arg eq "skipRight")			{	our $skipRight = 1;		}
	elsif	($arg eq "skipBackward")		{	our $skipBackward = 1;	}
	elsif	($arg eq "skipLeft")			{	our $skipLeft = 1;		}
	elsif	($arg eq "skipUp")				{	our $skipUp = 1;		}
	elsif	($arg eq "skipDown")			{	our $skipDown = 1;		}
	elsif	($arg =~ /\d.*,\d/)				{	our $resolution = $arg;	}
	else									{	our $miscArg = $arg;	}
}

#setup render outputs for baking (diff,norm,ao,surfaceID)
sub setupRenderOutputs{
	my $itemCount = lxq("query sceneservice item.n ? all");
	my @renderOutputs;
	
	#query if the filepaths should be created as well.
	my $specifyFilePaths = quickDialog("Set up the filepaths for the outputs?",boolean,0,"","");
	if ($specifyFilePaths == 1){
		our $filePath = lxq("query sceneservice scene.file ? current");
		if ($filePath eq ""){die("Can't specify the file paths for the renderoutputs because the scene hasn't been saved yet and doesn't have a filename");}
		$filePath =~ s/\..*//;
	}
	
	#query if diffuse should be linear or not
	my $linearDiff = quickDialog("Should the diffuse map be linear?",boolean,0,"","");
	
	#delete preexisting render outputs
	for (my $i=0; $i<$itemCount; $i++){
		if (lxq("query sceneservice item.type ? $i") eq "renderOutput"){
			my $id = lxq("query sceneservice item.id ? $i");
			push(@renderOutputs,$id);
		}
	}
	lx("!!select.drop item");
	lx("!!select.subItem {$_} add textureLayer;render;environment;light;camera;scene;replicator;mediaClip;txtrLocator") for @renderOutputs;
	lx("!!texture.delete");
	...
	lx("select.subItem {$renderOutputs[0]} set textureLayer;render;environment;light;camera;scene;replicator;mediaClip;txtrLocator");
	lx("!!shader.setEffect mat.diffuse");
	if ($linearDiff == 1)	{	lx("!!item.channel renderOutput\$colorspace \"nuke-default:linear\"");	}
	else					{	lx("!!item.channel renderOutput\$colorspace \"nuke-default:sRGB\"");	}
	if ($specifyFilePaths == 1){
		my $path = $filePath . "_diffBake";
		lx("!!item.channel renderOutput\$filename {$path}");
		if ($linearDiff == 1)	{	lx("!!item.channel renderOutput\$format TIF16");	}
		else					{	lx("!!item.channel renderOutput\$format \$Targa");	}
		lx("!!item.editorColor red");
	}
	#ao
	lx("!!shader.create renderOutput");
	@renderOutputs = lxq("query sceneservice selection ? renderOutput");
	lx("select.subItem {$renderOutputs[0]} set textureLayer;render;environment;light;camera;scene;replicator;mediaClip;txtrLocator");
	lx("!!shader.setEffect occl.ambient");
	lx("!!item.channel renderOutput\$colorspace \"nuke-default:linear\"");
	lx("!!item.channel renderOutput\$occlRays 256");
	if ($specifyFilePaths == 1){
		my $path = $filePath . "_aoBake";
		lx("!!item.channel renderOutput\$filename {$path}");
		lx("!!item.channel renderOutput\$format \$Targa");
		lx("!!item.editorColor red");
	}
	#shading normal
	lx("!!shader.create renderOutput");
	...
	lx("!!item.channel renderOutput\$colorspace \"nuke-default:linear\"");
	lx("!!item.channel renderOutput\$remap true");
	if ($specifyFilePaths == 1){
		my $path = $filePath . "_normalBake";
		lx("!!item.channel renderOutput\$filename {$path}");
		lx("!!item.channel renderOutput\$format \$Targa");
		lx("!!item.editorColor red");
	}
	...
	@renderOutputs = lxq("query sceneservice selection ? renderOutput");
	lx("select.subItem {$renderOutputs[0]} set textureLayer;render;environment;light;camera;scene;replicator;mediaClip;txtrLocator");
	lx("!!shader.setEffect geo.surface");
	if ($specifyFilePaths == 1){
		my $path = $filePath . "_idBake";
		lx("!!item.channel renderOutput\$filename {$path}");
		lx("!!item.channel renderOutput\$format \$Targa");
		lx("!!item.editorColor red");
	}
	...
	@renderOutputs = lxq("query sceneservice selection ? renderOutput");
	lx("select.subItem {$renderOutputs[0]} set textureLayer;render;environment;light;camera;scene;replicator;mediaClip;txtrLocator");
	lx("!!shader.setEffect shade.illum");
	lx("!!shader.setEffect shade.alpha");
	if ($specifyFilePaths == 1){
		my $path = $filePath . "_alphaBake";
		lx("!!item.channel renderOutput\$filename {$path}");
		lx("!!item.channel renderOutput\$format \$Targa");
		lx("!!item.editorColor red");
	}
...
sub focalLenTweak{
	...
	lx("!!select.subItem {$cameraID} add mesh;camera;light;txtrLocator;backdrop;groupLocator;replicator;surfGen;locator;deform;locdeform;deformGroup;deformMDD2;morphDeform;itemInfluence;genInfluence;deform.wrap;softLag;ABCCurvesDeform.sample;ABCdeform.sample;chanModify;chanEffect;defaultShader;defaultShader 0 0");
	if ($miscArg eq "down")	{	lx("!!item.channel camera\$target ?*0.75");	}
	else					{	lx("!!item.channel camera\$target ?*1.25");	}
}
...
sub fovTweak{
	...
	my $cameraID = lxq("render.camera ?");
	lx("!!select.subItem {$cameraID} add mesh;camera;light;txtrLocator;backdrop;groupLocator;replicator;surfGen;locator;deform;locdeform;deformGroup;deformMDD2;morphDeform;itemInfluence;genInfluence;deform.wrap;softLag;ABCCurvesDeform.sample;ABCdeform.sample;chanModify;chanEffect;defaultShader;defaultShader 0 0");
	if ($miscArg eq "down")	{	lx("!!item.channel camera\$focalLen ?*0.75");	}
	else					{	lx("!!item.channel camera\$focalLen ?*1.25");	}
}

#light editor
sub lightEditor{
	#if (lxq("query sceneservice selection ? camera") < 1){
		#my $renderCamera = lxq("render.camera ?");
		#lxout("renderCamera = $renderCamera");
		#lx("!!select.subItem {$renderCamera} add mesh;meshInst;camera;light;txtrLocator;backdrop;groupLocator;replicator;surfGen;locator;deform;locdeform;deformGroup;deformMDD2;morphDeform;itemInfluence;genInfluence;deform.wrap;softLag;ABCdeform.sample;chanModify;chanEffect;defaultShader;defaultShader 0 0");
	#}
	
	#drop current lightmaterial selection
	my @lightMatSel = lxq("query sceneservice selection ? lightMaterial");
	lx("select.subItem {$_} remove textureLayer;render;environment;light;camera;scene;replicator;mediaClip;txtrLocator") for @lightMatSel;

	#select currently selected lights' lightmaterials
	my @lightSel = lxq("query sceneservice selection ? light");
	foreach my $id (@lightSel){
		my $name = lxq("query sceneservice item.name ? {$id}");
		my @children = lxq("query sceneservice item.children ? {$id}");
		foreach my $child (@children){
			if (lxq("query sceneservice item.type ? $child") eq "lightMaterial"){
				lx("!!select.subItem {$child} add textureLayer;render;environment;light;camera;scene;replicator;mediaClip;txtrLocator");
			}
		}
	}
	...
	#show form
	lx("!!attr.formPopover {83865340060:sheet}");
}


#turn off render region
sub turnOffRenderRegion{
	lx("!!select.itemType polyRender");
	lx("!!item.channel polyRender\$region false");
}

#set render region from selected polys' uv bounds
sub setRenderReg_selUVs{
	my $mainlayer = lxq("query layerservice layers ? main");
	my @polys = lxq("query layerservice polys ? selected");
	lx("!!select.itemType polyRender");
	lx("!!select.type polygon");
	
	#if no polys selected, then turn off render region
	if (@polys == 0){
		lx("item.channel polyRender\$region false");
		lxout("[->] : Turning off render region because no polys are selected");
		return;
	}
	
	#if some polys selected, set render region to uv bounds.
	selectVmap();
	my $vertCount = lxq("query layerservice poly.numVerts ? $polys[0]");
	my @vmapValues = lxq("query layerservice poly.vmapValue ? $polys[0]");
	my @uvBounds = ($vmapValues[0],$vmapValues[1],$vmapValues[0],$vmapValues[1]);
	...
	foreach my $poly (@polys){
		...
		for (my $i=0; $i<$vertCount; $i++){
			...
			if ($vmapValues[$indice1] < $uvBounds[0])	{	$uvBounds[0] = $vmapValues[$indice1];	}
			if ($vmapValues[$indice2] < $uvBounds[1])	{	$uvBounds[1] = $vmapValues[$indice2];	}
			if ($vmapValues[$indice1] > $uvBounds[2])	{	$uvBounds[2] = $vmapValues[$indice1];	}
			if ($vmapValues[$indice2] > $uvBounds[3])	{	$uvBounds[3] = $vmapValues[$indice2];	}
		}
	}
	
	#flip uv bounds vertical axis.
	$uvBounds[1] = $uvBounds[1] * -1 + 1;
	$uvBounds[3] = $uvBounds[3] * -1 + 1;
	
	lx("!!item.channel polyRender\$region true");
	lx("!!item.channel polyRender\$regX0 {$uvBounds[0]}");
	lx("!!item.channel polyRender\$regX1 {$uvBounds[2]}");
	lx("!!item.channel polyRender\$regY0 {$uvBounds[1]}");
	lx("!!item.channel polyRender\$regY1 {$uvBounds[3]}");
	
	lxout("[->] : Setting the render regions to this : {@uvBounds}");
}
...
sub bakeEachImage{
	...
	my @files = fileDialog("save","Directory where to save the file:","*.tga","tga");
	$files[0] =~ s/\\/\//g;
	$files[0] =~ s/\/[a-zA-Z0-9-_()\.\s]+$//;
	
	#find selected images and put 'em in a list
	for (my $i=0; $i<$txLayerCount; $i++){
		if ((lxq("query sceneservice txLayer.isSelected ? $i") == 1) && (lxq("query sceneservice txLayer.type ? $i") eq "imageMap")){
			my $type = lxq("query sceneservice txLayer.type ? $i");
			lxout("type = $type");
			push(@imageIDs,lxq("query sceneservice txLayer.id ? $i"));
		}
	}

	#setup diffuse coefficient output
	for (my $i=0; $i<$txLayerCount; $i++){
		if (lxq("query sceneservice txLayer.type ? $i") eq "renderOutput"){
			push(@renderOutputIDs,lxq("query sceneservice txLayer.id ? $i"));
		}
	}
	my $foundDiffOutput = 0;
	foreach my $id (@renderOutputIDs){
		if (lxq("item.channel effect {?} set {$id}") eq "mat.diffuse"){
			$foundDiffOutput = 1;
			last;
		}
	}
	if ($foundDiffOutput == 0){
		lx("select.itemType polyRender");
		lx("!!shader.create renderOutput");
		lx("!!shader.setEffect mat.diffuse");
		lx("item.channel renderOutput\$colorspace \"nuke-default:sRGB\"");
	}
	foreach my $id (@renderOutputIDs){
		my $effect = lxq("item.channel effect {?} set {$id}");
		if (($effect eq "mat.diffuse") || ($effect eq "shade.alpha")){
			lx("shader.setVisible item:{$id} visible:{1}");
		}else{
			lx("shader.setVisible item:{$id} visible:{0}");
		}
	}
	
	#bake out images
	foreach my $id (@imageIDs){
		lx("!!shader.setVisible item:{$_} visible:{0}") for @imageIDs;
		lx("!!shader.setVisible item:{$id} visible:{1}");
		my $name = lxq("query sceneservice txLayer.name ? {$id}");
		$name =~ s/\s*\(Image\)//i;
		$name =~ s/\s*\([0-9]+\)//;
		my $filePath = $files[0] . "\/" . $name;
		lx("!!bake filename:{$filePath} format:{PNG}");
		if (lxres != 0){	die("The user hit the cancel button");	}
	}
	
	lx("!!shader.setVisible item:{$imageIDs[-1]} visible:{0}");
}
...
#render sky box
sub renderSkyBox{
	#if you type in 1024,1024 (or any other number,number) as an arg it'll use that res (and only pay attention to the first number)
	if ($resolution == 0){	
		$resolution = quickDialog("Render Resolution",integer,128,16,16384);	
		if (lxres != 0){		die("The user hit the cancel button");}	
	}else{
		$resolution =~ s/,.*//;
	}
	
	#file save dialog (and skip if you typed in a miscArg argument)
	if ($miscArg eq ""){
		...
		lx("dialog.open");
		if (lxres != 0){	die("The user hit the cancel button");	}
		our $filePath = lxq("dialog.result ?");
	}else{
		our $filePath = $miscArg;
	}
	...
	if		($filePath =~ /_forward/i)		{	$filePath =~ s/_forward//i;		}
	elsif	($filePath =~ /_right/i)		{	$filePath =~ s/_right//i;		}
	elsif	($filePath =~ /_backward/i)		{	$filePath =~ s/_backward//i;	}
	elsif	($filePath =~ /_left/i)			{	$filePath =~ s/_left//i;		}
	elsif	($filePath =~ /_up/i)			{	$filePath =~ s/_up//i;			}
	elsif	($filePath =~ /_down/i)			{	$filePath =~ s/_down//i;		}
	...
	#render frames
	if ($skipForward != 1)	{	renderSkyBoxFrame(0,0,0,$filePath,"_forward",$fileExt);		}
	if ($skipRight != 1)	{	renderSkyBoxFrame(0,270,0,$filePath,"_right",$fileExt);		}
	if ($skipBackward != 1)	{	renderSkyBoxFrame(0,180,0,$filePath,"_backward",$fileExt);	}
	if ($skipLeft != 1)		{	renderSkyBoxFrame(0,90,0,$filePath,"_left",$fileExt);		}
	if ($skipUp != 1)		{	renderSkyBoxFrame(90,0,0,$filePath,"_up",$fileExt);			}
	if ($skipDown != 1)		{	renderSkyBoxFrame(-90,0,0,$filePath,"_down",$fileExt);		}
}

#render a skybox image
sub renderSkyBoxFrame{
	lx("transform.channel rot.X {$_[0]}");
	lx("transform.channel rot.Y {$_[1]}");
	lx("transform.channel rot.Z {$_[2]}");
	my $fileName = $_[3] . $_[4];
	my $format = returnModoSaveFormat($_[5]);

	if ((-e $fileName) && (!-w $fileName)){system("p4 edit \"$fileName\"") or lxout("fail to check this out : $fileName");}
	lx("render.visible filename:{$fileName} format:{$format}");
}
...
sub selectAllByType{
	...
	my %itemList;
		...
		$itemList{genInfluence} = 1;
		$itemList{"deform.lattice"} = 1;
...
sub scaleLighting{
	...
	#scale selected lights
	if ($miscArg eq "selected"){
		my @selLightIDs = lxq("query sceneservice selection ? light");
		foreach my $id (@selLightIDs){
			my $brightness = lxq("item.channel light\$radiance {?} set {$id}") * $lightingScale;
			lx("!!item.channel light\$radiance {$brightness} set {$id}");
		}
...
sub setGamma{
	...
	#find all diffuse maps (and env map) and change their gamma
	...
	foreach my $id (@txLayers){
		...
		if ((lxq("shader.setEffect ?") eq "diffColor") || (lxq("shader.setEffect ?") eq "envColor")){
...
#set the number of occlusion rays for all materials in the scene
sub occlusionTexRays{
	lxout("[->] Setting the number of rays for all the occlusion textures in the scene.");
	my $value;
	if ($miscArg =~ /[0-9]/){
		$value = $miscArg;
	}else{	
		lx("user.value sene_occlusionRays");
		$value = lxq("user.value sene_occlusionRays ?");
		if 		($value < 1)	{	$value = 1;	}
		elsif 	($value > 512)	{	popup("are you sure you want to tell all the occlusion textures to use ($value) occlusion rays?!");	}
	}
	
	my $txLayers = lxq("query sceneservice txLayer.n ?");
	for (my $i=0; $i<$txLayers; $i++){
		my $type = lxq("query sceneservice txLayer.type ? $i");
		if ($type eq "occlusion"){
			my $id = lxq("query sceneservice txLayer.id ? $i");
			lx("item.channel item:{$id} name:{occlusion\$rays} value:{$value}");
		}
	}
}
...
sub blurryRefRays{
	...
	if ($miscArg =~ /[0-9]/){
		...
		$value = lxq("user.value sene_blurryRays ?");
		if 		($value < 1)	{	$value = 1;	}
...
sub rememberACTRSettings{
	...
	if ($skipActr != 1){
		...
		else
		{
			...
			else												{	 $actr = 1;  $selAxis = "auto";			lxout("You were using an action AXIS that I couldn't read so I'm loading AUTO");}
			...
			else												{ 	 $actr = 1;  $selCenter = "select";		lxout("You were using an action CENTER that I couldn't read so I'm loading SEL-CENTER");}
		}
