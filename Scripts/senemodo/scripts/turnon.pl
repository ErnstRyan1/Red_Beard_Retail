...
sub restoreACTR{
	...
	if ($selCenter == 1){
		lx("tool.set actr.worldAxis on");
	}elsif ($selCenter == 2){
		lx("tool.set actr.localAxis on");
	}elsif ($selCenter == 3){
		lx("tool.set actr.parentAxis on");
	}else{
...
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#REMEMBER SELECTION SETTINGS (modo8) (modded)
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
sub rememberACTR{
	lxout("[->] Remembering the current ACTR");
	
	if   ( lxq( "tool.set actr.auto ?") eq "on")			{	$selCenter = "auto";			$selAxis = "auto";			}
	elsif( lxq( "tool.set actr.select ?") eq "on")			{	$selCenter = "select";			$selAxis = "select";		}
	elsif( lxq( "tool.set actr.border ?") eq "on")			{	$selCenter = "border";			$selAxis = "select";		}
	...
	elsif( lxq( "tool.set actr.parent ?") eq "on")			{	$selCenter = "parent";			$selAxis = "parent";		}
	...
	elsif( lxq( "tool.set actr.pivotparent ?") eq "on")		{	$selCenter = "pivot";			$selAxis = "parent";		}
	
	elsif( lxq( "tool.set actr.worldAxis ?") eq "on")		{	$selCenter = 1;					$selAxis = 1;				}
	elsif( lxq( "tool.set actr.localAxis ?") eq "on")		{	$selCenter = 2;					$selAxis = 2;				}
	elsif( lxq( "tool.set actr.parentAxis ?") eq "on")		{	$selCenter = 3; 				$selAxis = 3;				}
	...
	#if no actr was currently loaded, turn on selectauto
	if ($selCenter eq ""){ $selCenter = "select"; $selAxis = "auto";}
