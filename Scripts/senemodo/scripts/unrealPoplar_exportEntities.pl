...
foreach my $id (@meshInsts){
	...
	#if ($ignoreRot == 0){	for (my $i=0; $i<@worldRot; $i++){	$worldRot[$i] = roundDecimal($worldRot[$i],6);	}	}	#ue4 = float
	if ($ignoreRot == 0){	for (my $i=0; $i<@worldRot; $i++){	$worldRot[$i] = int($worldRot[$i]*182 + .5);	}	}	#ue3 = 182 x rotation float
	...
	#print text
	$text .= "      Begin Actor Class=StaticMeshActor Name=StaticMeshActor_".$count." Archetype=StaticMeshActor'Engine.Default__StaticMeshActor'\n";
	$text .= "         Begin Object Class=StaticMeshComponent Name=StaticMeshComponent0 ObjName=StaticMeshComponent_1 Archetype=StaticMeshComponent'Engine.Default__StaticMeshActor:StaticMeshComponent0'\n";
	$text .= "            StaticMesh=StaticMesh'Test_MilitaryConcept.Meshes.SM_".$name."'\n";										#
	$text .= "            VertexPositionVersionNumber=1\n";																			#
	$text .= "            Name=\"StaticMeshComponent_1\"\n";																#
	$text .= "            ObjectArchetype=StaticMeshComponent'Engine.Default__StaticMeshActor:StaticMeshComponent0'\n";				#
	$text .= "         End Object\n";
	$text .= "         StaticMeshComponent=StaticMeshComponent'StaticMeshComponent_1'\n";											#
	$text .= "         Components(0)=StaticMeshComponent'StaticMeshComponent_1'\n";													#
	
	#poplar
	$worldPos[2] *= -1;
	if ($ignorePos == 0){	$text .= "         Location=(X=".$worldPos[2].",Y=".$worldPos[0].",Z=".$worldPos[1].")\n";			}	#
	if ($ignoreRot == 0){	$text .= "         Rotation=(Pitch=".$worldRot[0].",Yaw=".$worldRot[2].",Roll=".$worldRot[1].")\n";	}	#
	
	#if ($ignorePos == 0){	$text .= "         Location=(X=".$worldPos[0].",Y=".$worldPos[2].",Z=".$worldPos[1].")\n";			}	#
	#if ($ignoreRot == 0){	$text .= "         Rotation=(Pitch=".$worldRot[1].",Yaw=".$worldRot[2].",Roll=".$worldRot[0].")\n";	}	#
	if ($ignoreScl == 0){	$text .= "         DrawScale3D=(X=".$worldScl[0].",Y=".$worldScl[2].",Z=".$worldScl[1].")\n";		}	#
#	$text .= "         CreationTime=79242.359375\n";																				#
#	$text .= "         Tag="StaticMeshActor"\n";																					#
#	$text .= "         CollisionComponent=StaticMeshComponent'StaticMeshComponent_62'\n";											#
#	$text .= "         EditorQuat=(X=0.295755,Y=0.450521,Z=0.342989,W=0.769362)\n";													#
#	$text .= "         ActorGuid=(A=2405317491,B=1082509950,C=923200429,D=3407514299)\n";											#
	$text .= "         Name=\"StaticMeshActor_".$count."\"\n";																				#
#	$text .= "         ObjectArchetype=StaticMeshActor'Engine.Default__StaticMeshActor'\n";											#
