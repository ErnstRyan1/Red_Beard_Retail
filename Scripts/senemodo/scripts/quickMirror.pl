...
userValueTools(sen_mirrorMerge,integer,config,"Merge verts when mirroring","Use Cvars;Force Merge;Force No Merge","","",xxx,xxx,"",0);
my $merge = 1;
if ($noMerge == 1){	$merge = 0;	}
if		(lxq("user.value sen_mirrorMerge ?") eq "Force Merge")		{	$merge = 1;	}
elsif	(lxq("user.value sen_mirrorMerge ?") eq "Force No Merge")	{	$merge = 0;	}
...
#------------------------------------------------------------------------------------------------------------
	...
	if ($centeringToolsScriptPos ne ""){
		...
		if		(@ARGV[0] =~ /x/i)	{lx("poly.mirror axis:[x] merge:$merge cenX:{$pos[0]} cenY:{0} cenZ:{0}");}
		elsif	(@ARGV[0] =~ /y/i)	{lx("poly.mirror axis:[y] merge:$merge cenX:{0} cenY:{$pos[1]} cenZ:{0}");}
		else						{lx("poly.mirror axis:[z] merge:$merge cenX:{0} cenY:{0} cenZ:{$pos[2]}");}
		lxout("Mirroring using the fake origin position (@pos) stored from the 'centeringTools.pl' script instead of the world origin.  If you wish to mirror from the world origin, reset the other script's cvar by running this command : '@centeringTools.pl resetPos'");
	}else{
...
sub mirror{
	...
	if ($alg == 301){
		...
		lx("tool.reset");
		lx("tool.attr poly.mirror merge $merge");
		...
		lx("tool.set *.mirror on");
		lx("tool.reset");
		lx("tool.attr effector.clone merge $merge");
		if 		($mirrorAxis == 0)	{	our @axis = (1,0,0,0,1,0);					}
		elsif	($mirrorAxis == 1)	{	our @axis = (0,1,0,0,0,1);					}
		else						{	our @axis = (0,0,1,1,0,0);					}
